...
class PublishError(RuntimeError):
    """Safe publishing failure that never embeds credentials or response bodies."""


class ResultPublisher:
    """The single outbound transport used by future model services."""
    ...
    async def _post(self, path: str, payload: dict) -> None:
        try:
            ...
            raise PublishError(
                f"Backend rejected AI Worker request with status {exc.response.status_code}"
            ) from exc
