...
**建议标题：** 隐私约束下的实时音频诈骗证据分析。节点：RTSP AAC→PyAV→16k mono PCM→RMS端点→SenseVoiceSmall ONNX中文转写与文本规范化→有效文本校验→60秒上下文→规则/关键组合与可选Qwen并行→证据衰减/滞回→四状态→脱敏摘要→M4。标出原始音频不落盘、ASR与LLM均在本地运行，且Ollama不承载ASR模型。
