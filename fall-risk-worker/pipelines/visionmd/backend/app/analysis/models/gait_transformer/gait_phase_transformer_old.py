...
class PositionalEncodingLayer(layers.Layer):
    ...
    def call(self, inputs):
        # The temporal dimension is intentionally dynamic in the functional
        # model.  TensorFlow 2.17 therefore exposes ``inputs.shape[1]`` as
        # ``None`` while tracing; use the runtime shape instead.
...
def get_gait_phase_stride_transformer(
    ...
    flat_inputs = layers.Lambda(lambda x: tf.reshape(x, [tf.shape(x)[0], tf.shape(x)[1], num_joints * joint_dim]))(x)
    encoded_poses = layers.Dense(units=projection_dim, name="embedding")(flat_inputs)
    ...
    # This model's published weights are loaded through legacy Keras.  Keep the
    # graph compatible with tf-keras 2.17 instead of relying on Keras 3 ops.
    repeated_demographics = layers.Lambda(
        lambda value: tf.tile(value, [1, 10, 1])
    )(encoded_demographics)
    encoded_features = layers.Concatenate(axis=1)(
        [repeated_demographics, encoded_poses]
    )
